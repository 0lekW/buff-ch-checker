...
Promise.all([
  fetch(chrome.runtime.getURL('blue_gem_data.json')).then(response => response.json()),
  chrome.storage.local.get('akMetric')
]).then(([data, storage]) => {
  blueGemData = data;
  if (storage.akMetric) {
...
function processListings() {
  // Find all listing rows (both regular listings and bookmarks)
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  if (areaName === 'local' && changes.akMetric) {
